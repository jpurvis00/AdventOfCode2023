...
namespace Trebuchet
{
    internal static class Part1
    {
        public static void DayOnePartOne()
        {
            ...
            int firstCalibrationNumb = -1;
            int lastCalibrationNumb = -1;
            ...
            foreach (string line in fileText)
            {
                for (int i = 0; i < line.Length; i++)
                {
                    ...
                    if (int.TryParse(line[i].ToString(), out int isNumb) && firstCalibrationNumb == -1)
                    {
                        firstCalibrationNumb = isNumb;
                        continue;
                    }

                    /* finds any other #s in the string */
                    if (int.TryParse(line[i].ToString(), out isNumb) && firstCalibrationNumb != -1)
                    {
                        lastCalibrationNumb = isNumb;
                    }
                }

                /* if only 1 # is found in the string, use that # for the 2nd # */
                if (lastCalibrationNumb == -1)
                {
                    lastCalibrationNumb = firstCalibrationNumb;
                }

                /* join the first # with the second number and add to the total */
                calibrationTotal += int.Parse(firstCalibrationNumb.ToString() + lastCalibrationNumb.ToString());

                /* reset values for the next input string */
                firstCalibrationNumb = -1;
                lastCalibrationNumb = -1;
            }

            Console.WriteLine($"Part 1 Calibration Total: {calibrationTotal}");
        }
